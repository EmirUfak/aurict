...
async function load(workdir: string): Promise<TodoItem[]> {
  try {
    return JSON.parse(await readFile(todoPath(workdir), "utf8")) as TodoItem[];
  } catch {
    return [];
  }
}
...
export const todoTool: ToolDef = {
  id: "todo",
  description:
    "Manage a project-local todo list. Use the action field exactly, e.g. { action: 'add', text: 'Write tests' }. Actions: list, add, complete, delete.",
  parameters: z.object({
    action: z
    ...
    id: z
      ...
      .optional()
      .describe(
        "Todo ID prefix (first 6 chars) — required for complete/delete",
      ),
  }),
