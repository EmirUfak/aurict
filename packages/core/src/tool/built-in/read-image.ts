...
into the transcript. Supports PNG, JPEG, WebP, and GIF up to 5 MB.`,
  ...
  async execute(args, ctx) {
    ...
    let absolute: string
    try {
      absolute = await resolveAuthorizedFilesystemPath(ctx.workdir, ctx.sessionId, requested)
    } catch (error) {
      return { output: "", error: `Security: Path is outside workspace or not approved: ${error instanceof Error ? error.message : String(error)}` }
    }
