...
import { normalizeKnownToolArgs } from "../src/tool/executor.js";
...
describe("todo list", () => {
  it("normalizes the documented list alias only when it is a valid todo action", () => {
    expect(
      normalizeKnownToolArgs("todo", { list: "add", text: "Write chapter" }),
    ).toEqual({
      list: "add",
      action: "add",
      text: "Write chapter",
    });
    expect(normalizeKnownToolArgs("todo", { list: "unknown" })).toEqual({
      list: "unknown",
    });
  });
...
describe("todo complete", () => {
  ...
  it("persists done status to disk", async () => {
    ...
    const data = JSON.parse(
      readFileSync(join(tmpDir, ".aurict", "todos.json"), "utf8"),
    );
