...
// useProviders() already wires its own useSidecarRecovery internally, so a
// ModelSelector mounted on any screen re-fetches (and its toast, when
// showToast is passed, auto-clears) the same way MainScreen's provider list
// does — no extra wiring needed here.
interface Props {
  showToast?: (message: string, tone: ToastItem['tone'], action?: ToastItem['action']) => number;
  dismissToast?: (id: number) => void;
}

export function ModelSelector({ showToast, dismissToast }: Props = {}) {
  const providers = useProviders(); const selection = useModelSelection();
  const error = providers.error ?? selection.error;
  useErrorToast(providers.error, providers.errorSeq, showToast ? providers.refresh : null, showToast ?? (() => -1), dismissToast);
